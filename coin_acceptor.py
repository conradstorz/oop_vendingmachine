...
class MyCoinAcceptor(CoinAcceptor):
    ...
    def setAcceptableCoins(self, acceptable_coins_dict):
        ...
        msg = Message(
            src=1, dst=self.addr, header=Message.HEADER_MOD_INHIBIT, data=bitarray.bytes
        )
        response = self.iface.send(msg)
    ...
    def setInhibitOff(self):
        ...
        msg = Message(
            src=1, dst=self.addr, header=Message.HEADER_MOD_MASTER_INHIBIT, data=b"\x01"
        )
    ...
    def setInhibitOn(self):
        ...
        msg = Message(
            src=1, dst=self.addr, header=Message.HEADER_MOD_MASTER_INHIBIT, data=b"\x00"
        )
        self.pause_polling = True
        time.sleep(0.2)
        self.iface.send(msg)
